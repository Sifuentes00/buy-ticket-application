# Диаграммы последовательностей

# Содержание
1. [Вход в учётную запись](#1)
2. [Регистрация в приложении](#2)
3. [Покупка билетов](#3)
4. [Оставление отзыва](#4)


### 1. Вход в учётную запись<a name="1"></a>

![Вход в учётную запись](https://github.com/Sifuentes00/buy-ticket-application/blob/main/docs/Diagrams/Images/SequenceDiagramSignIn.png)

### 2. Регистрация в приложении<a name="2"></a>

![Регистрация в приложении](https://github.com/Sifuentes00/buy-ticket-application/blob/main/docs/Diagrams/Images/SequenceDiagramSignUp.png)
  
### 3. Добавление фильма<a name="3"></a>

![Покупка билетов](https://github.com/Sifuentes00/buy-ticket-application/blob/main/docs/Diagrams/Images/SequenceBuyTicket.png)

### 4. Оставление отзыва<a name="3"></a>

![Оставление отзыва](https://github.com/Sifuentes00/buy-ticket-application/blob/main/docs/Diagrams/Images/SequenceReview.png)